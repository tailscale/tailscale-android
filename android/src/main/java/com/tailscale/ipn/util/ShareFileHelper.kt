...
object ShareFileHelper : libtailscale.ShareFileHelper {
  ...
  fun init(context: Context, uri: String, appScope: CoroutineScope) {
    ...
    TSLog.d("KARI", "init ShareFileHelper with savedUri: $savedUri")
  }
  ...
  override fun renamePartialFile(
    ...
    try {
      ...
      TSLog.d("KARI", "renamePartialFile with uri: $partialUri and dir: $targetDirUri")
      ...
      val targetDir =
          DocumentFile.fromTreeUri(context, targetDirUriObj)
              ?: throw IllegalStateException("Invalid target directory URI: $targetDirUri")
      ...
      destFile =
          targetDir.createFile("application/octet-stream", finalTargetName)
              ?: throw IOException("Failed to create destination file: $finalTargetName")

      context.contentResolver.openInputStream(partialUriObj)?.use { input ->
        context.contentResolver.openOutputStream(destFile.uri)?.use { output ->
          input.copyTo(output)
        } ?: throw IOException("Failed to open output stream for $finalTargetName")
      } ?: throw IOException("Failed to open input stream from $partialUri")
