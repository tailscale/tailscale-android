...
class App : Application(), libtailscale.AppContext {
    ...
    override fun onCreate() {
        ...
        applicationScope.launch {
            Notifier.tileReady.collect { isTileReady ->
                setTileReady(isTileReady)
            }
        }
    }
    ...
    fun connect() {
        val callback: (Result<Ipn.Prefs>) -> Unit = { result ->
            if (result.isSuccess) {
                ...
                prefs?.let {
                    Log.d("App", "Connect: preferences updated successfully: $prefs")
                } ?: Log.d("App", "Connect: No prefs available, but call was successful.")
            } else {
                val error = result.exceptionOrNull()
                Log.e("App", "Failed to update preferences: ${error?.message}")
            }
        }
        setWantRunning(true, callback)
    }

    private fun setWantRunning(wantRunning: Boolean, callback: (Result<Ipn.Prefs>) -> Unit) {
        Ipn.MaskedPrefs().WantRunning = wantRunning
        Client(applicationScope).editPrefs(Ipn.MaskedPrefs(), callback)
    }
    ...
    private fun setAndRegisterNetworkCallbacks() {
        connectivityManager.requestNetwork(dnsConfigObj.dnsConfigNetworkRequest, object : ConnectivityManager.NetworkCallback() {
            override fun onAvailable(network: Network) {
                super.onAvailable(network)
                val sb = StringBuilder()
                val linkProperties: LinkProperties? = connectivityManager.getLinkProperties(network)
                val dnsList: MutableList<InetAddress> = linkProperties?.getDnsServers()
                        ?: mutableListOf()
                for (ip in dnsList) {
    ...
    fun requestWriteStoragePermission(act: Activity) {
        ...
        act.requestPermissions(arrayOf(Manifest.permission.WRITE_EXTERNAL_STORAGE), IPNActivity.WRITE_STORAGE_RESULT)
    }
    ...
    fun notifyFile(uri: String?, msg: String?) {
        ...
        val pending: PendingIntent = PendingIntent.getActivity(this, 0, viewIntent, PendingIntent.FLAG_UPDATE_CURRENT)
        val builder: NotificationCompat.Builder = NotificationCompat.Builder(this, FILE_CHANNEL_ID)
    ...
    override fun getInterfacesAsString(): String {
        val interfaces: ArrayList<NetworkInterface> = java.util.Collections.list(NetworkInterface.getNetworkInterfaces())
    ...
    fun getSyspolicyBooleanValue(key: String?): Boolean? {
        val manager: RestrictionsManager = this.getSystemService(Context.RESTRICTIONS_SERVICE) as RestrictionsManager
