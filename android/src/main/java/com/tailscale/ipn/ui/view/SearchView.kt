...
fun SearchView(
  ...
  Log.d("KARI", "BackStackEntry autoFocus: $autoFocus")

  // Use TextFieldValue to preserve text and cursor position.
  var searchFieldValue by
      rememberSaveable<TextFieldValue>(stateSaver = TextFieldValue.Saver) {
  ...
  // Extra effect to force the cursor to the end when the search field is active.
  LaunchedEffect(expanded, searchFieldValue.text) {
    if (expanded &&
        searchFieldValue.text.isNotEmpty() &&
        searchFieldValue.selection.start != searchFieldValue.text.length) {
      delay(100) // Allow UI to settle
      searchFieldValue = searchFieldValue.copy(selection = TextRange(searchFieldValue.text.length))
    }
  }
